...
| **周报导入** | 上传正式周报Markdown → BOARD_SYNC解析 → 自动同步 | [docs/modules/WeeklyReportImport.md](docs/modules/WeeklyReportImport.md) |
| **重复检测** | 稳定事项键去重，重复事项更新原记录 | [docs/modules/DuplicateDetection.md](docs/modules/DuplicateDetection.md) |
...
年度及季度目标直接修改 `frontend/src/config/goals.json`，重新构建后生效。
